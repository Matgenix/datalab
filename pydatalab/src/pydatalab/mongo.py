...
def resolve_tags_for_docs(docs: list[dict]) -> None:
    """Inline managed tag details into each doc's `tags` field, in place.
    ...
    Free-text string tags pass through unchanged, and references to deleted tags
    are dropped.
    """
    ...
    for doc in docs:
        for tag in doc.get("tags") or []:
            if isinstance(tag, dict):
                raw_id = tag.get("immutable_id", tag.get("_id"))
                if raw_id is not None:
                    tag_ids.add(ObjectId(raw_id))
    ...
    resolved = {
        doc["_id"]: doc
        for doc in flask_mongo.db.tags.find(
            {"_id": {"$in": list(tag_ids)}},
    ...
    for doc in docs:
        ...
        for tag in tags:
            ...
            if isinstance(tag, dict):
                raw_id = tag.get("immutable_id", tag.get("_id"))
                if raw_id is None:
                    # No identifier: an inlined/custom entry; keep as-is.
                    resolved_tags.append(tag)
                    continue
                match = resolved.get(ObjectId(raw_id))
...
def create_default_indices(
    ...
    ret += db.tags.create_index("creator_ids", name="tag creator_ids", background=background)
