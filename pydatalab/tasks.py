...
# names a directory under `pydatalab/example_data/` whose files are globbed at
# run time (so new example files are picked up automatically), minus any
# `exclude` fnmatch patterns for files the parser cannot handle. Groups are
# expanded by `_build_seed_items` into one seeded item per file (a single block
# per item), so a fresh dev instance exercises every block type that has
# example data. Groups flagged `single_item` instead produce one item carrying
# the whole sorted file list -- used for multi-file blocks (uv-vis: the
# alphabetically-first file is the background) and file-less blocks (comment).
_SEED_BLOCK_GROUPS: list[dict] = [
    ...
    },
    {
        ...
        "item_type": "cells",
        "dir": "echem",
        # .mps/.sta are Biologic companion files to the .mpr data, not
        # standalone datasets.
        "exclude": ["README", "*.mps", "*.sta"],
    },
    {
        "block_type": "nmr",
        "item_type": "samples",
        "dir": "NMR",
        # The block only renders 1D data: 72.zip is a 2D dataset (metadata
        # only, no plot) and multi_nuclei.zip selects an experiment with no
        # Bruker binary. Gzipped JEOL files are not accepted on upload.
        "exclude": ["README", "72.zip", "multi_nuclei.zip", "*.gz"],
    },
    {
        "block_type": "ftir",
        "item_type": "samples",
    ...
    },
    {
        ...
        "item_type": "samples",
        "dir": "TGA-MS",
        # The ms parser decodes as UTF-8 and this file contains a latin-1
        # degree sign, raising a decode error.
        "exclude": ["mp2028_281122_LNO+C.txt"],
    },
    {
        "block_type": "tabular",
        "item_type": "samples",
    ...
    {
        # A single uv-vis block needs >=2 files; the first (alphabetically,
        # 1908047U1_0000) is the background scan.
        "block_type": "uv-vis",
        "item_type": "samples",
        "single_item": True,
    ...
    {
        # File-less block (no `dir`): the text lives in the `freeform_comment` field.
        "block_type": "comment",
        "item_type": "samples",
        "single_item": True,
...
def _make_item_id(base: str) -> str:
    """Slugify `base`, truncate to the item-id length limit and append a short suffix.

    The suffix comes from an RNG seeded with `base` itself, so each id is stable
    across runs and independent of the other seeded items -- re-running `dev.seed`
    (or adding new example files) never renames existing items, keeping the task
    idempotent. It also disambiguates ids whose slugs would otherwise collide
    after truncation.
    """
    rng = random.Random(base)  # noqa: S311 - not cryptographic, just stable id suffixes
    suffix = "".join(rng.choices(string.ascii_lowercase + string.digits, k=4))
    stem = _slug(base)[: _MAX_ITEM_ID_LENGTH - len(suffix) - 1].rstrip("_")
    return f"{stem}_{suffix}"


def _list_group_files(group: dict, example_data: pathlib.Path) -> list[str]:
    """Sorted `example_data`-relative paths of the group's directory, minus `exclude` patterns."""
    if "dir" not in group:
        return []
    exclude = group.get("exclude", [])
    return sorted(
        f"{group['dir']}/{path.name}"
        for path in (example_data / group["dir"]).iterdir()
        if path.is_file() and not any(fnmatch.fnmatch(path.name, pattern) for pattern in exclude)
    )


def _build_seed_items(example_data: pathlib.Path) -> list[dict]:
    ...
    for group in _SEED_BLOCK_GROUPS:
        ...
        files = _list_group_files(group, example_data)
        ...
        if group.get("single_item"):
            ...
            items.append(
                {
                    "payload": {
                        "item_id": _make_item_id(f"seed_{block_type}"),
                        "type": item_type,
                        "name": name,
        ...
        for rel_path in files:
            ...
            items.append(
                {
                    "payload": {
                        "item_id": _make_item_id(f"seed_{block_type}_{filename}"),
                        "type": item_type,
...
def _post_seed_item(client, payload: dict) -> str:
    """Create an item via the API; returns "created", "exists" or "failed"."""
    ...
    if resp.status_code not in (200, 201):
        print(f"  ! {item_id}: create failed ({resp.status_code}): {resp.get_data(as_text=True)}")
        return "failed"
    print(f"  + {item_id}: created")
    return "created"


def _upload_example_files(
    client, item_id: str, rel_paths: list[str], example_data: pathlib.Path
) -> list[str]:
    """Upload each example file and attach it to the item, returning the file ids.

    Order is preserved: the first file is the background scan for multi-file blocks.
    """
    file_ids: list[str] = []
...
def _attach_rendered_block(
    client, item_id: str, block_type: str, file_ids: list[str], extra_block_data: dict
) -> None:
    """Create a data block on the item, wire in the uploaded file(s) and render it."""
    add = client.post(
        "/add-data-block/",
    ...
    block_data.update(extra_block_data)
...
def seed(_):
    ...
    with app.test_client() as client:
        for spec in _build_seed_items(example_data):
            item_id = spec["payload"]["item_id"]
            status = _post_seed_item(client, spec["payload"])
            if status == "exists":
                skipped += 1
            elif status == "created":
                created += 1
                if spec["block_type"] is not None:
                    file_ids = _upload_example_files(client, item_id, spec["files"], example_data)
                    _attach_rendered_block(
                        client, item_id, spec["block_type"], file_ids, spec["block_data"]
                    )
