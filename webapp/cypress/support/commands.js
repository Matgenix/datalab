...
Cypress.Commands.add("createTagViaAPI", (data) => {
  // data: { name, description?, color?, creator_ids?, group_ids? }. Returns the new tag's id.
  return cy
    .request({
      method: "PUT",
      url: API_URL + "/tags",
...
Cypress.Commands.add("deleteTagByNameViaAPI", (name) => {
  // Best-effort cleanup: delete every tag visible to the current user with this name.
