...
describe("TagColorPicker.vue", () => {
  it("renders the preset palette and a no-color option", () => {
    cy.mount(TagColorPicker, { props: { modelValue: null } });
    cy.get(".swatch:not(.swatch-none)").should("have.length", TAG_COLOR_PALETTE.length);
    cy.get(".swatch-none").should("exist");
  });
  ...
  it("emits null for the no-color option", () => {
    cy.mount(TagColorPicker, {
      props: { modelValue: TAG_COLOR_PALETTE[0], "onUpdate:modelValue": cy.spy().as("update") },
    });
    cy.get(".swatch-none").click();
