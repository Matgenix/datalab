<template>
  ...
  >
    ...
    <template #option="{ name, immutable_id, color, description, scope }">
      <span
        class="tag-option"
      ...
      >
        ...
        <span>{{ name }}</span>
        <span v-if="scope" class="badge badge-pill badge-light text-uppercase ml-1 text-primary">{{
          scope
        }}</span>
    ...
    <template #selected-option="{ name, immutable_id, color, description }">
      <span v-if="color" class="color-swatch" :style="{ backgroundColor: color }"></span>
      <span
        :class="{ 'free-text-tag': !immutable_id }"
        :title="description || (immutable_id ? 'Managed tag' : 'Free-text tag')"
        >{{ name }}</span
      >
    </template>
  ...
  computed: {
    ...
    value: {
      ...
      set(newValue) {
        ...
        // re-resolved on every read, so this is harmless denormalisation (like
        // `name`); `scope` is per-user/context and is never persisted.
    ...
    tagOptions() {
      ...
      // free-text tags by name: a managed tag must not be matched by name, since
      // same-named tags can coexist in different scopes.
